...
W_FLOOR_CLEAR_BONUS     = 700_000  # stronger push for full evacuation   # NEW: reward fully clearing floors
W_FLOOR_DENSITY         = 200_000   # NEW: prioritize dense floors
...
# NEW: Floor clear feasibility scoring
floor_clear_feasible = {}
for (b,f), occ in floor_current_occ.items():
    capacity = floor_seat_capacity.get((b,f),0)
    free_elsewhere = sum(
        1 for e in emp_ids
        if (e_bldg[e], e_floor[e]) == (b,f)
        and len(emp_valid[e]) > 1
    )
    # feasible if enough employees can move out
    floor_clear_feasible[(b,f)] = (free_elsewhere >= occ * 0.8)
...
def run_greedy():
    ...
    def evac_priority(e):
        # Enhanced prioritization for freeing large & dense floors
        floor_key = (e_bldg[e], e_floor[e])
        floor_size = floor_seat_capacity.get(floor_key, 0)
        current_occ = floor_current_occ.get(floor_key, 0)
        density = current_occ / floor_size if floor_size > 0 else 0

        return (
            0 if (e_bldg[e],e_floor[e]) in tier1_set else 1,
            0 if floor_clear_feasible.get((e_bldg[e], e_floor[e]), False) else 1,
            0 if (e_bldg[e],e_floor[e]) in tier1_set else 1,
            -density,
            -floor_size,
            -current_occ,
    ...
    for e in order:
        ...
        # NEW: scoring boost for seats that help evacuate large floors
        
        # NEW: lookahead penalty - avoid blocking others
        def blocking_penalty(ri):
            future_block = 0
            for other in emp_ids:
                if other == e:
                    continue
                if ri in emp_valid[other] and len(emp_valid[other]) <= 2:
                    future_block += 1
            return future_block

        def floor_evacuate_score(ri):
            key = (s_bldg[ri], s_floor[ri])
            size = floor_seat_capacity.get(key, 0)
            occ  = floor_current_occ.get(key, 0)
            return size * 2 + occ - blocking_penalty(ri) * 50

        if cands:
            cands = sorted(cands, key=lambda ri: -floor_evacuate_score(ri))
            cands = set(cands)
...
if CP_TIME_LIMIT_SEC > 0:
    ...
    if min_floors is not None and _PHASE2_TIME > 0:
        ...
        def _tsw(t):
            ...
            return (
            0 if (e_bldg[e],e_floor[e]) in tier1_set else 1,
            0 if floor_clear_feasible.get((e_bldg[e], e_floor[e]), False) else 1,int(W_TEAM_FLOOR_SPREAD*(ts**0.7))
