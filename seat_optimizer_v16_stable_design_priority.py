...
W_FLOOR_CLEAR_BONUS     = 800_000
W_DESIGN_ROOM_PRIORITY  = 600_000
...
def run_greedy():
    ...
    def evac_priority(e):
        ...
        is_design = floor_key in design_floors

        return (
            0 if is_design else 1,
            0 if (e_bldg[e],e_floor[e]) in tier1_set else 1,
            -floor_size,0 if (e_bldg[e],e_floor[e]) in tier1_set else 1,
...
if CP_TIME_LIMIT_SEC > 0:
    ...
    # DESIGN FLOOR PENALTY (SAFE)
    design_penalty = []
    for (e, ri) in x:
        if (s_bldg[ri], s_floor[ri]) in design_floors:
            design_penalty.append(x[(e, ri)])
    ...
    if min_floors is not None and _PHASE2_TIME > 0:
        ...
        def _tsw(t):
            ...
            0 if is_design else 1,
            0 if (e_bldg[e],e_floor[e]) in tier1_set else 1,
            -floor_size,int(W_TEAM_FLOOR_SPREAD*(ts**0.7))
        ...
        model.Minimize(sum(
            ...
            + [W_LARGE_FLOOR_EVAC*v for v in large_floor_residual.values()] + [DESIGN_PRIORITY_WEIGHT * v for v in design_penalty]
        ))
        
    DESIGN_PRIORITY_WEIGHT = 3_000_000
