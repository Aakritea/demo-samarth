"""
╔══════════════════════════════════════════════════════════════════════╗
║      Enterprise Seat Optimisation  —  Production v9.0               ║
║      Windfall Power  |  Multi-Plant Seat Reallocation                ║
║      FLOOR-MINIMISATION PRESSURE POINT RELEASE                       ║
╚══════════════════════════════════════════════════════════════════════╝

CHANGES FROM v8.0 → v9.0  (10 pressure-point fixes)
══════════════════════════════════════════════════════

  P1-FIX  PHASE-2 FLOOR CONSTRAINT (CRITICAL BUG)
           Phase 1 ObjectiveValue = weighted_floors + thin_penalty +
           residual_penalty. Phase 2 constrained sum(floor_w) ≤ that
           inflated value, making the bound completely toothless.
           Fix: record p1_weighted_floor_val separately (before
           penalties are added) and use THAT as the Phase 2 ceiling.

  P2-FIX  REMOVE _lg_w DEAD CODE (RUNTIME BUG)
           _lg_w referenced final before it existed (NameError).
           The function was never called — Phase 2 used W_LARGE_FLOOR_EVAC
           directly. Removed entirely.

  P3-FIX  PHASE-1 TIME RAISED 200 s → 500 s
           Weighted objective has wider LP gap. 200 s rarely finds the
           true minimum. Extra time here pays back more than in Phase 2.

  P4-FIX  team_on_fl LINKING — AddMaxEquality REPLACES O(n) CLAUSES
           for av in assigned: model.Add(tof >= av) added one clause
           per employee per (team, floor). Replaced with a single
           model.AddMaxEquality call — same semantics, far fewer clauses.

  P5-FIX  SMALL-TEAM HARD FLOOR CAP IN CP-SAT
           C730 at 20 % lets a team of 5 occupy 5 separate floors
           (min_count = 1). Added a hard CP-SAT cap: teams ≤ 10 → 2
           floors max, teams 11-19 → 3 floors max. Keeps small teams
           consolidated without violating spec.

  P6-FIX  GREEDY — DYNAMIC TEAM FLOOR PREFERENCE
           team_pref_floor was static (computed from original seats
           before any assignments). Replaced with a live running dict
           floor_running_count[team][floor] updated after each
           assignment. Subsequent teammates are pulled toward where
           the team is actually landing, not where it started.

  P7-FIX  FLOOR-MERGER REPAIR PASS ADDED
           After main loop + C730 repair, a new merger sweep identifies
           floors where an entire team sub-group is below thin_threshold
           and tries to move them to the team's dominant floor, freeing
           the source floor entirely.

  P8-FIX  mgr_diff — SIMPLIFIED ENCODING
           Previous: |pairs| × |floors| bool vars. New: one IntVar per
           employee tracking assigned floor-index; manager proximity
           tested with a linear abs-difference constraint. Dramatically
           fewer variables, same soft objective.

  P9-FIX  PRE-FIX UNREACHABLE floor_used TO 0
           For floors where no employee in emp_valid has a reachable
           seat, floor_used[f] is fixed to 0 before solving. Eliminates
           CP-SAT propagation work on provably-zero vars.

  P10-FIX REMOVE DUPLICATE THIRD POISONING PASS
           The third large-floor poisoning loop was logically identical
           to the first and added zero new employees. Removed.

CARRIED FROM v8.0
──────────────────
  RE-OPT-1  Evac targeting by RealEstateScore (vacant × total_seats).
  RE-OPT-2  Capacity-weighted Phase 1 floor objective.
  RE-OPT-3  Capacity-proportional greedy evac penalty.
  RE-OPT-4  C730 unified at 20 %.
  RE-OPT-5  Excluded-unit variant spellings.
  RE-OPT-6  W_LARGE_FLOOR_EVAC = 2 000 000.
  FIX-C1    Zero seat-type violations.
  FIX-C5    CAB/CUB ±2-floor tolerance.

CHANGES FROM v7.4 → v8.0  (Real-Estate Optimisation)
═══════════════════════════════════════════════════════

  CORE PHILOSOPHY SHIFT — "LARGE FLOOR PRIORITY"
  ────────────────────────────────────────────────
  Previous strategy: evacuate the SMALLEST occupied floors first
  (fewest employees to move = easiest). This leaves large, high-value
  floors half-empty, wasting the most expensive real estate.

  New strategy: evacuate the LARGEST floors that are already partially
  vacant first. Metric = total_seats × vacancy_ratio (= vacant seat count).
  A 300-seat floor that is 70% vacant scores 210 vs 9 for a 30-seat floor
  that is 70% vacant — correctly prioritising the bigger real-estate gain.

  RE-OPT-1  EVAC TARGET SELECTION — vacancy-volume metric
             Replace EVAC_MAX_OCCUPIED (hard occupied-count cap) with:
             • EVAC_MAX_OCC_RATIO    — occupancy ratio ceiling (e.g. 0.55)
             • EVAC_MIN_VACANCY_RATIO — vacancy ratio floor   (e.g. 0.45)
             Floors sorted descending by (total_seats × vacancy_ratio),
             i.e. those with the most vacant seats are targeted first.
             EVAC_MAX_FLOORS raised to 20 to capture more large floors.

  RE-OPT-2  PHASE-1 WEIGHTED FLOOR OBJECTIVE
             CP-SAT Phase 1 previously minimised sum(floor_used[f])
             — treating a 10-seat floor and a 300-seat floor equally.
             Now minimises sum(floor_capacity_weight[f] × floor_used[f])
             where weight = ceil(total_seats / median_floor_capacity).
             Freeing a large floor contributes proportionally more to
             the objective, so the solver prioritises large-floor evac.

  RE-OPT-3  GREEDY SCORE — CAPACITY-PROPORTIONAL FLOOR PENALTY
             score() now adds a negative term proportional to the
             target floor's total seat capacity when that floor is an
             evac target. Large evac floors are avoided more strongly
             than small ones.

  RE-OPT-4  C7_30 THRESHOLD UNIFIED AT 20%
             Per specification: "post shifting a team should be in the
             same building with minimum 20% on the same floor."
             C730_LARGE_TEAM_PCT and C730_SMALL_TEAM_PCT both set to 0.20.
             Previously small teams had 0.30, which over-restricted
             movement and made it harder to consolidate large floors.

  RE-OPT-5  EXCLUDED UNITS — VARIANT SPELLINGS ACCEPTED
             EXCLUDED_UNITS now includes both "Shindewadi Plant" and
             "Kesurdi Plant" plus bare variants without the "Plant"
             suffix, guarding against inconsistent source-data naming.

  RE-OPT-6  LARGE-FLOOR EVAC WEIGHT INCREASED
             W_LARGE_FLOOR_EVAC raised from 500 000 to 2 000 000 to
             make the CP-SAT Phase-2 penalty for leaving employees on
             a high-value evac floor much stronger.

  CARRIED FROM v7.4
  ──────────────────
  FIX-C1  Zero seat-type violations via type-safe greedy fallback.
  FIX-C5  CAB/CUB manager proximity: ±2 floor tolerance.
  FIX-C3  Evac targets computed from Seat Allocation Vacant rows.
  FIX-C4  Unified Scorecard sheet.
"""
...
OUTPUT_FILE = "optimized_seat_allocation_v90.xlsx"
...
# RE-OPT-6: increased — leaving staff on a high-value evac floor costs
# proportionally more, pushing CP-SAT to prioritise large-floor clearance
W_LARGE_FLOOR_EVAC     = 2_000_000

# ── RE-OPT-1: Real-estate floor evacuation strategy ─────────────────
# Strategy: target LARGE floors that are already mostly VACANT.
# Metric  : total_seats × vacancy_ratio  (= vacant seat count).
# A 300-seat floor at 70 % vacancy (score 210) outranks a 30-seat floor
# at 100 % vacancy (score 30) — correctly prioritising real-estate value.
# Filters are ratio-based so large floors qualify regardless of raw count.
EVAC_MAX_OCC_RATIO         = 0.55  # floor occupancy ratio must be ≤ 55 %
EVAC_MIN_VACANCY_RATIO     = 0.45  # floor vacancy  ratio must be ≥ 45 %
EVAC_MIN_FREE_RATIO        = 0.40  # ≥ 40 % of occupants must be bldg-moveable
EVAC_MAX_FLOORS            = 20    # raised cap — capture more large floors
MIN_ALT_SEATS_AFTER_POISON = 1

# Big-team floor consolidation
BIG_TEAM_MIN_SIZE          = 20
MAX_FLOORS_BIG_TEAM_HARD   = 3
MAX_FLOORS_MED_TEAM_HARD   = 4

MIN_FLOOR_PCT           = 0.20
THIN_THRESHOLD          = 15
MAX_FLOORS_PER_BIG_TEAM = 5
# RE-OPT-4: Unified 20 % floor-presence minimum for ALL team sizes.
# Spec: "post shifting, a team should be in the same building with
#        minimum 20 % on the same floor."
C730_LARGE_TEAM_PCT     = 0.20
C730_SMALL_TEAM_PCT     = 0.20   # was 0.30 — now aligned with spec
C730_SIZE_BOUNDARY      = 20

# FIX-C5: manager floor tolerance (0 = same floor only, 2 = ±2 floors)
MGR_FLOOR_TOLERANCE = 2

CP_TIME_LIMIT_SEC = 1800
_PHASE1_TIME      = 500   # P3-FIX: raised 200→500; weighted obj needs more time
_PHASE2_TIME      = CP_TIME_LIMIT_SEC - _PHASE1_TIME
CP_WORKERS        = 8

# P5-FIX: hard floor-count caps for small teams in CP-SAT
# C730 at 20% allows team-of-5 → 5 floors. These caps close that loophole.
MAX_FLOORS_SMALL_TEAM_HARD = 2   # teams ≤ SMALL_TEAM_BOUNDARY
MAX_FLOORS_TINY_TEAM_HARD  = 1   # teams ≤ TINY_TEAM_BOUNDARY  (single floor)
SMALL_TEAM_BOUNDARY        = 10
TINY_TEAM_BOUNDARY         = 4

# RE-OPT-5: Accept both "Plant" suffix and bare unit names to guard
# against inconsistent source-data naming across input files.
EXCLUDED_UNITS  = {
    "Shindewadi Plant", "Shindewadi",
    "Kesurdi Plant",    "Kesurdi",
    "Kesundi Plant",    "Kesundi",   # alternate spelling seen in briefs
}
...
SEP = "=" * 64
print(f"\n{SEP}")
print("  Enterprise Seat Optimisation  --  Production v9.0  [Pressure-Point Release]")
print(f"{SEP}\n")
...
# ───────────────────────────────────────────────────────────────────
# 9.  RE-OPT-1: REAL-ESTATE FLOOR EVACUATION TARGETING
#
#     PREVIOUS: sort by Occupied ascending — targets the SMALLEST floors
#               (easiest to clear but lowest real-estate value).
#
#     NEW: sort by (total_seats × vacancy_ratio) descending.
#          = number of ALREADY-VACANT SEATS on the floor.
#          A 300-seat floor at 70 % vacancy scores 210;
#          a 30-seat floor at 100 % vacancy scores 30.
#          This correctly prioritises high-capacity floors with lots of
#          already-empty desks — maximum space reclaimed per employee moved.
#
#     Filters (ratio-based, not count-based):
#       • OccRatio   ≤ EVAC_MAX_OCC_RATIO      (not too full to clear)
#       • VacRatio   ≥ EVAC_MIN_VACANCY_RATIO  (already substantially empty)
#       • FreeRatio  ≥ EVAC_MIN_FREE_RATIO     (enough moveable employees)
# ───────────────────────────────────────────────────────────────────
...
# Total seats per floor (Occupied + Vacant)
floor_stats["TotalSeats"] = floor_stats["Occupied"] + floor_stats["Vacant"]

# Occupancy and vacancy ratios
floor_stats["OccRatio"] = floor_stats.apply(
    lambda r: r["Occupied"] / r["TotalSeats"] if r["TotalSeats"] > 0 else 1.0, axis=1)
floor_stats["VacRatio"] = 1.0 - floor_stats["OccRatio"]

# RE-OPT-1 real-estate score: vacant seat count (= TotalSeats × VacRatio)
# Higher = more empty space on a larger floor = bigger real-estate win
floor_stats["RealEstateScore"] = floor_stats["TotalSeats"] * floor_stats["VacRatio"]

# Bldg-moveable employee count per floor
...
# Select evac targets using ratio-based filters, then rank by RealEstateScore
evac_candidates = floor_stats[
    (floor_stats["Occupied"] > 0) &
    (floor_stats["OccRatio"] <= EVAC_MAX_OCC_RATIO) &
    (floor_stats["VacRatio"] >= EVAC_MIN_VACANCY_RATIO) &
    (floor_stats["FreeRatio"] >= EVAC_MIN_FREE_RATIO)
].sort_values("RealEstateScore", ascending=False).head(EVAC_MAX_FLOORS)

tier1_floors: list = [(row["Building"], row["Floor"]) for _, row in evac_candidates.iterrows()]
tier1_set = set(tier1_floors)

# Build floor_vac display table (sorted by RealEstateScore)
floor_vac = floor_stats.sort_values("RealEstateScore", ascending=False).reset_index(drop=True)
...
print(f"\n{'─'*64}")
print(f"  EVAC TARGETS  — Real-Estate Priority  (OccRatio≤{EVAC_MAX_OCC_RATIO:.0%}, VacRatio≥{EVAC_MIN_VACANCY_RATIO:.0%})")
print(f"  Sorted by RealEstateScore = TotalSeats × VacancyRatio  (descending)")
print(f"{'─'*64}")
for (b, f) in tier1_floors:
    row = evac_candidates[(evac_candidates["Building"]==b)&(evac_candidates["Floor"]==f)]
    if not len(row): continue
    total_n = int(row["TotalSeats"].values[0])
    occ_n   = int(row["Occupied"].values[0])
    vac_n   = int(row["Vacant"].values[0])
    free_n  = int(row["FreeCnt"].values[0])
    score_v = float(row["RealEstateScore"].values[0])
    print(f"  {b} | {f}: Total={total_n}, Occ={occ_n} ({100*occ_n/max(total_n,1):.0f}%), "
          f"Vac={vac_n} ({100*vac_n/max(total_n,1):.0f}%), BldgFree={free_n}, Score={score_v:.1f}")
...
for e in emp_ids:
    ...
    if len(pv) >= MIN_ALT_SEATS_AFTER_POISON:
        emp_valid[e] = pv; large_poisoned.add(e)
# P10-FIX: third loop removed — it was logically identical to the first
# (same tier1_set check, same condition) and added zero new employees.

print(f"  Large-poisoned          : {len(large_poisoned)}")
print(f"  Safety-valve            : {len(large_unpoisoned)}")
print(f"{'─'*64}")

# RE-OPT-2 / RE-OPT-3: pre-compute per-floor capacity weight for CP-SAT
# objective and greedy penalty.  Weight = ceil(cap / median_cap) so a
# large floor contributes proportionally more to the evacuation objective.
_all_caps = [c for c in floor_seat_capacity.values() if c > 0]
_median_cap = sorted(_all_caps)[len(_all_caps)//2] if _all_caps else 1
floor_capacity_weight: dict = {
    (b, f): max(1, math.ceil(floor_seat_capacity.get((b,f), 1) / _median_cap))
    for b in bldgs for f in floors
}
# Also index by floor name alone (for greedy score lookup by s_floor[ri])
floor_name_capacity: dict = defaultdict(int)
for (b, f), cap in floor_seat_capacity.items():
    floor_name_capacity[f] = max(floor_name_capacity[f], cap)
_max_floor_cap = max(floor_name_capacity.values()) if floor_name_capacity else 1
...
def run_greedy():
    ...
    # P6-FIX: dynamic team floor preference — updated live after each assignment
    # so teammates are pulled toward where the team is *actually* landing,
    # not where it started. Initialised from current positions as a warm seed.
    floor_running_count: dict = defaultdict(lambda: defaultdict(int))
    for e in emp_ids:
        floor_running_count[e_team[e]][e_floor[e]] += 1

    def team_pref_floor_live(team):
        fc = floor_running_count[team]
        return max(fc, key=fc.get) if fc else ""
    ...
    for e in order:
        ...
        _tf     = team_pref_floor_live(e_team[e])   # P6-FIX: live query
        _on_lg  = (e_bldg[e],e_floor[e]) in tier1_set

        def score(ri, _e=e, _pf=partner_floors, _g=_g,
                  _tub=_tub, _tf=_tf, _on_lg=_on_lg):
            sc = random.random()
            # RE-OPT-3: penalty proportional to floor capacity so that
            # assigning to a large evac floor is avoided much more strongly
            # than a small one — mirrors the real-estate priority logic.
            if (s_bldg[ri],s_floor[ri]) in tier1_set:
                cap_penalty = 100_000 * (floor_name_capacity.get(s_floor[ri],1) / _max_floor_cap)
                sc -= cap_penalty
        ...
        floor_running_count[e_team[e]][s_floor[best]] += 1  # P6-FIX: update live
    ...
    # P7-FIX: FLOOR-MERGER SWEEP
    # For each team, find floors where the team has < min_count employees
    # AND the entire group on that floor can be moved to the team's dominant
    # floor, eliminating the source floor entirely (or at least reducing it
    # below thin_threshold so CP-SAT can clean up the rest).
    merger_moves = merger_fail = floors_merged = 0
    merged_floors: set = set()
    for t in teams:
        ts = team_size.get(t,0)
        if ts == 0: continue
        t_emps = [e for e in emp_ids if e_team[e]==t]
        ...
        for e in t_emps:
            floor_mbrs[s_floor[assignment[e]]].append(e)
        if len(floor_mbrs) <= 1: continue  # already on one floor

        # Dominant = floor with most team members (excluding tier1)
        non_evac_floors = {f: ms for f, ms in floor_mbrs.items()
                           if (e_bldg[t_emps[0]], f) not in tier1_set and len(ms) > 0}
        if not non_evac_floors: continue
        dominant_f = max(non_evac_floors, key=lambda f: len(non_evac_floors[f]))

        min_count = max(1, int(team_c730_threshold.get(t, 0.20) * ts))
        for src_f, mbs in list(floor_mbrs.items()):
            if src_f == dominant_f: continue
            if len(mbs) >= min_count: continue        # already compliant, skip
            if (e_bldg[t_emps[0]], src_f) in tier1_set: continue  # already evac target
            # Try to move entire sub-group to dominant floor
            moved_here = 0
            for e in list(mbs):
                stype = e_stype.get(e,"CL")
                seats = [ri for ri in (emp_valid[e] & available)
                         if s_floor[ri] == dominant_f
                         and s_type[ri] == stype
                         and (s_bldg[ri], s_floor[ri]) not in tier1_set]
                if seats:
                    chosen = seats[0]
                    available.add(assignment[e])
                    available.discard(chosen)
                    old_f = s_floor[assignment[e]]
                    assignment[e] = chosen
                    floor_mbrs[old_f].remove(e)
                    floor_mbrs[dominant_f].append(e)
                    merger_moves += 1
                    moved_here += 1
                else:
                    merger_fail += 1
            if moved_here == len(mbs):
                merged_floors.add(src_f)
                floors_merged += 1

    print(f"  P7 floor-merger: {merger_moves} moves, {floors_merged} floors merged, {merger_fail} unresolved")
...
if CP_TIME_LIMIT_SEC > 0:
    ...
    # P9-FIX: pre-compute which floors are reachable by at least one employee
    reachable_floors: set = set()
    for e in emp_ids:
        for ri in emp_valid[e]:
            reachable_floors.add(s_floor[ri])

    floor_used = {f: model.NewBoolVar(f"fl_{fl_idx[f]}") for f in floors}
    # P9-FIX: fix unreachable floors to 0 — eliminates useless inference
    for f in floors:
        if f not in reachable_floors:
            model.Add(floor_used[f] == 0)
    ...
    # P4-FIX: use AddMaxEquality instead of O(n) individual ≥ clauses.
    # model.AddMaxEquality(target, [v1,v2,...]) ≡ target = max(v1,v2,...).
    # For booleans this means tof is 1 iff any assignment var on that floor is 1.
    for ti,t in enumerate(teams):
        ...
        for f in floors:
            ...
            if not assigned:
                model.Add(team_on_fl[t,f] == 0)
            else:
                # tof = 1 iff any member is on this floor (max of bool vars)
                model.AddMaxEquality(team_on_fl[t,f], assigned)
    ...
    for t in teams:
        ...
        # P5-FIX: tiered hard caps — small/tiny teams get tighter limits
        if ts <= TINY_TEAM_BOUNDARY:
            cap_val = min(max(min_req, 1), MAX_FLOORS_TINY_TEAM_HARD)
        elif ts <= SMALL_TEAM_BOUNDARY:
            cap_val = min(max(min_req, 1), MAX_FLOORS_SMALL_TEAM_HARD)
        elif ts < C730_SIZE_BOUNDARY:
    ...
    print(f"\nPhase 1: minimise capacity-weighted floors ({_PHASE1_TIME}s) ...")
    # RE-OPT-2: weight each floor_used[f] by its seat capacity so that
    # freeing a large floor contributes proportionally more to the objective
    # than freeing a tiny one.  This is the core real-estate priority change.
    _floor_w_terms = []
    for f in floors:
        # find representative building for this floor (use max capacity)
        best_b = max(bldgs, key=lambda b: floor_seat_capacity.get((b,f),0))
        w = floor_capacity_weight.get((best_b, f), 1)
        _floor_w_terms.append(w * floor_used[f])
    model.Minimize(sum(_floor_w_terms)
                   + sum(thin_vars)
    ...
    if min_floors is not None and _PHASE2_TIME > 0:
        # RE-OPT-2: constraint on weighted floor sum, not raw count
        model.Add(sum(_floor_w_terms) <= min_floors)
        def _tsw(t):
            ts = team_size.get(t,0)
            return (int(W_TEAM_FLOOR_SPREAD*(ts**0.7))
                    if W_TEAM_FLOOR_SPREAD_SCALE and ts>1 else W_TEAM_FLOOR_SPREAD)
        # RE-OPT-6: scale large-floor evac penalty by floor capacity weight
        def _lg_w(e):
            f = s_floor[final.get(e, list(emp_valid[e])[0])] if emp_valid.get(e) else None
            b = s_bldg[final.get(e, list(emp_valid[e])[0])] if emp_valid.get(e) else None
            return W_LARGE_FLOOR_EVAC * floor_capacity_weight.get((b, f), 1) if f and b else W_LARGE_FLOOR_EVAC
        model.Minimize(sum(
...
# Floor Summary — enriched with real-estate metrics
...
# Add capacity and real-estate score columns for reference
fs["TotalSeats"] = fs.apply(
    lambda r: floor_seat_capacity.get((r["Building"],r["Floor"]),0), axis=1)
fs["VacantSeats"] = fs["TotalSeats"] - fs["Before"]
fs["OccRatio_Before"] = fs.apply(
    lambda r: round(r["Before"]/r["TotalSeats"],3) if r["TotalSeats"]>0 else 0, axis=1)
fs["RealEstateScore"] = fs["TotalSeats"] * (1 - fs["OccRatio_Before"])
fs = fs[["Building","Floor","Before","After","Delta","Status",
         "TotalSeats","VacantSeats","OccRatio_Before","RealEstateScore"]]
fs = fs.sort_values(["Status","RealEstateScore"],ascending=[True,False]).reset_index(drop=True)
...
total_seats_freed = sum(
    floor_seat_capacity.get((r["Building"], r["Floor"]), 0)
    for _, r in fs[fs["Status"]=="FREED"].iterrows()
)
...
scorecard_rows = [
    ...
    {"Constraint": "OBJ  Floors freed (count)",
     "Pass": int(floors_freed),   "Fail": int(floors_after),
     "Pass_Pct": str(int(floors_freed)),
     "Status": "OBJECTIVE"},

    {"Constraint": "OBJ  Total seats freed (real-estate)",
     "Pass": int(total_seats_freed), "Fail": int(len(seat_idx) - total_seats_freed),
     "Pass_Pct": f"{round(100*total_seats_freed/max(len(seat_idx),1),1)}%",
     "Status": "OBJECTIVE"},

    {"Constraint": "OBJ  Large-floor evac targets cleared",
...
print(f"\n{SEP}")
print("  RESULTS SUMMARY  v8.0  [Real-Estate Edition]")
print(f"{SEP}")
print(f"  Method                   : {method}")
print(f"  Floors before            : {floors_before}")
print(f"  Floors after             : {floors_after}  (freed: {floors_freed})")
print(f"  Total seats freed        : {total_seats_freed}  (real-estate value)")
print(f"  Freed floors             : {', '.join(freed_floors_list) or 'None'}")
print(f"  Evac floor targets ({len(tier1_floors)}) — sorted by RealEstateScore:")
